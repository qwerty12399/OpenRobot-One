...
在现有 STM32F407ZGT6、两块 IBT-2/BTS7960 和双编码器接线基础上，增加一个仅用于 H3 逐通道验收的串口诊断功能。测试必须由一次性授权触发，每次只允许一台电机按固定 5% PWM 运行 100 ms，并由 TIM6 中断独立强制停车。
...
TIM3 周期为 4199，因此 5% 占空比比较值固定为 210。上电、复位和异常状态下四路 PWM 比较值必须为 0，PC0-PC3 必须为低。
...
3. `PULSING`：仅一个电机、一个方向以 5% PWM 运行，最长 10 个 TIM6 周期。
...
| `MOTOR-PULSE LEFT FWD` | 左 RPWM=5%、左 LPWM=0，运行 100 ms |
| `MOTOR-PULSE LEFT REV` | 左 RPWM=0、左 LPWM=5%，运行 100 ms |
| `MOTOR-PULSE RIGHT FWD` | 右 RPWM=5%、右 LPWM=0，运行 100 ms |
| `MOTOR-PULSE RIGHT REV` | 右 RPWM=0、右 LPWM=5%，运行 100 ms |
...
4. 仅设置所选方向的 TIM3 比较值为 210，另一方向保持 0。
...
4. 发送 ARM 和左 FWD，仅左路产生一次 5%/100 ms 脉冲，自动停车并返回 TIM2 增量。
5. 左 REV、右 FWD、右 REV 逐项重复，每个台阶都重新 ARM。
6. 脉冲期间发送 STOP，应提前停车。
7. 分别在脉冲期间断开CH340、复位STM32和退出上位机，均不得产生持续输出。
8. 保存四次脉冲的串口原始输出、编码器增量、电流和视频证据。

通过标准：无授权不输出；任一授权只触发一台电机的一次脉冲；正常到期时输出持续时间为 100 ms，允许一个 TIM6 周期的测量分辨率；停止后四路 PWM 和四个 EN 全部归零；左右编码器均返回非零且方向可解释的增量。
